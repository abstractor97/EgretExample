module wy {
	export class PageScaleOut implements IPageSwitch {
		public constructor() {
		}
		...
		private doAnim(obj,  callBack?, thisObj?): void {
			...
			if(wy.PageSwitch.easeOut) {
				tw.to({ scaleX: 0, scaleY: 0 }, wy.PageSwitch.durationOut,wy.PageSwitch.easeOut).call(function () { wy.Tools.stop(obj); if(callBack){if(thisObj){callBack.call(thisObj, obj);}else{callBack()}} });
			} else {
				tw.to({ scaleX: 0, scaleY: 0 }, wy.PageSwitch.durationOut).call(function () { wy.Tools.stop(obj); if(callBack){if(thisObj){callBack.call(thisObj, obj);}else{callBack()}} });
			}
		}
	}
}